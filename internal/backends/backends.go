...
// (Required)
func (rs *ResourceSource) Get(name string) (*sdp.Item, error) {
...
// Threadsafe tells the parent source if this backend is safe to be accessed by
// many threads at once. Normally a simple backend will be threadsafe since it
// won't be sharing anything in memory between each invocation, however the
// resources that is is accessing under the hood might not be, for example the
// "yum" package manager can't be accessed in parallel so that could be considered
// not threadsafe (Required)
func (rs *ResourceSource) Threadsafe() bool {
	return true
}
...
// loaded (Optional)
func (rs *ResourceSource) Supported() bool {
	return true
}
